  ...
  return (
    <div className="relative grid grid-cols-2 md:mb-10 md:flex md:gap-2.5 xl:block xl:shrink-0 xl:basis-[255px]">
      ...
      {isOpen && (
        <div
          className={
            'fixed z-20 col-[1_/_-1] ml-32 flex h-full flex-col gap-2.5 overflow-y-auto bg-background p-6 pt-28 sm:col-[2_/_-1] md:static md:ml-0 md:flex md:flex-row md:items-stretch md:gap-2.5 md:p-0 xl:flex xl:flex-col xl:gap-0'
          }
        >
          <CategoriesMenu />
          <RoadmapMenu setIsOpen={setIsOpen} />
        </div>
      )}
      {!isOpen && (
        <div
